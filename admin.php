...
<head>
    ...
    <title>Dynamic Image Admin - Text Position Editor</title>
    ...
    <style>
        /* Load TTF fonts for live preview */
        @font-face {
        ...
        @font-face {
            font-family: 'CaveatTTF';
            src: url('fonts/Caveat.ttf') format('truetype');
            font-weight: normal;
            font-style: normal;
        }
        @font-face {
            font-family: 'GloriaHallelujahTTF';
...
<body class="bg-dark min-h-screen p-4 md:p-6">
                ...
                <div class="mb-6">
                    ...
                    <div class="mb-4">
                        ...
                        <select id="fontFamily" onchange="updatePreview()" class="w-full px-4 py-2 bg-dark border border-dark-light rounded-lg text-light focus:ring-2 focus:ring-primary focus:border-transparent">
                            ...
                            <option value="GloriaHallelujah">Gloria Hallelujah (Fun)</option>
                        </select>
                    </div>
    ...
    <script>
                ...
                if (result.success) {
                    ...
                    reader.onload = function(e) {
                        ...
                        img.onload = function() {
                            ...
                            // Update image ID to the newly uploaded one
                            document.getElementById('imageId').value = result.imageId;
                            document.getElementById('imageId').readOnly = true;
        ...
        function updatePreview(skipImageCheck = false) {
            // Clear canvas
            ...
            // Map font family to TTF font for preview
            const fontMap = {
                'OpenSans': 'OpenSansTTF',
                'Roboto': 'RobotoTTF',
                'Caveat': 'CaveatTTF',
                'GloriaHallelujah': 'GloriaHallelujahTTF'
            };
            const ttfFont = fontMap[fontFamily] || 'OpenSansTTF';

            // Draw text with shadow
            ctx.font = `${fontWeight} ${fontSize}px ${ttfFont}`;
        ...
        function updatePositionDisplay() {
            document.getElementById('posX').textContent = Math.round(textX);
            document.getElementById('posY').textContent = Math.round(textY);

            const relX = ((textX / canvas.width) * 100).toFixed(1);
            const relY = ((textY / canvas.height) * 100).toFixed(1);
            document.getElementById('posXRel').textContent = relX;
            document.getElementById('posYRel').textContent = relY;
        }
        ...
        canvas.addEventListener('mousedown', function(e) {
            ...
            dragOffsetX = e.clientX - rect.left - textX;
            dragOffsetY = e.clientY - rect.top - textY;
        ...
        canvas.addEventListener('touchstart', function(e) {
            ...
            dragOffsetX = touch.clientX - rect.left - textX;
            dragOffsetY = touch.clientY - rect.top - textY;
        });
        ...
        async function saveConfiguration() {
            ...
            try {
                ...
                if (result.success) {
                    ...
                    let localConfigs = JSON.parse(localStorage.getItem('dynamicImageConfigs') || '{}');
        ...
        async function loadSavedConfigs() {
            ...
            try {
                ...
                if (result.success && result.configs && Object.keys(result.configs).length > 0) {
                    // Server has configs
                    displayConfigs(result.configs);
                    return;
                }
            } catch (error) {
                console.log('Server configs not available, using localStorage');
            }

            // Fallback to localStorage
            const localConfigs = JSON.parse(localStorage.getItem('dynamicImageConfigs') || '{}');
            displayConfigs(localConfigs);
        }
        ...
        async function deleteConfiguration(imageId, event) {
            event.stopPropagation();

            if (!confirm(`Are you sure you want to delete configuration for Image ID ${imageId}?`)) {
                return;
            }

            try {
                const response = await fetch(`save-config.php?id=${imageId}`, {
                    method: 'DELETE'
                });
                const result = await response.json();
        ...
        async function loadConfiguration(imageId) {
            let config = null;
            ...
            if (!config) {
                const localConfigs = JSON.parse(localStorage.getItem('dynamicImageConfigs') || '{}');
                config = localConfigs[imageId];
            }

            if (!config) return;

            // Load configuration into form
            document.getElementById('imageId').value = config.imageId;
            ...
            // Resize canvas to saved dimensions if available
            if (config.canvasWidth && config.canvasHeight) {
                canvas.width = config.canvasWidth;
                canvas.height = config.canvasHeight;
            }

            // Calculate position based on saved relative position
            textX = (parseFloat(config.posXRel) / 100) * canvas.width;
            textY = (parseFloat(config.posYRel) / 100) * canvas.height;

            // Load the image for this configuration
            const img = new Image();
            img.onload = function() {
                currentImage = img;
                updatePreview();
            ...
            img.onerror = function() {
                // If image fails to load, draw a gradient background and then the text
                const gradient = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
                gradient.addColorStop(0, '#667eea');
                gradient.addColorStop(1, '#764ba2');
                ctx.fillStyle = gradient;
                ...
                // Draw text on the gradient background
                updatePreview(true);
            };

            // Try to load the image file (try jpg first)
            img.src = `images/${imageId}.jpg`;

            // Fallback: try loading png if jpg fails after a delay
            setTimeout(() => {
                if (!currentImage || (currentImage && !currentImage.complete)) {
                    ...
                    pngImg.onerror = function() {
                        // PNG also failed, show gradient with text
                        const gradient = ctx.createLinearGradient(0, 0, canvas.width, canvas.height);
                        gradient.addColorStop(0, '#667eea');
                        gradient.addColorStop(1, '#764ba2');
                        ctx.fillStyle = gradient;
